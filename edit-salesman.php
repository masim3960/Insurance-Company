...
<body>
  ...
  <?php
  $empno = $_GET['empno'];
  $query7 = "SELECT * FROM `areasofworking` WHERE `EmpNo`='$empno'";
...
if(isset($_POST["submit"])){
  $areas= $_POST["areasofworking"];
  $areasofworking = explode(",",$areas);
  $query1 = "DELETE FROM `areasofworking` WHERE `EmpNo`='$empno'";
  $result1 = mysqli_query($con,$query1);
  foreach($areasofworking as $areaofworking){
    $query4 = "INSERT INTO `areasofworking`(`EmpNo`, `Areas`) VALUES ('$empno','$areaofworking')";
...
<h1 id="signup">Edit Salesman Information</h1>
<form class="row g-3" method="post">
  <div class="col-md-6">
    <label for="inputAreasOfWorking" class="form-label">Areas Of Working</label>
    <input type="text" name="areasofworking" class="form-control" id="inputAreasOfWorking" value="<?=implode(",",$areas)?>" required>
  </div>
