...
<body>
  ...
  <?php
  $empno = $_GET['empno'];
  $query7 = "SELECT * FROM `agent` WHERE `EmpNo`='$empno'";
  $result7 = mysqli_fetch_assoc(mysqli_query($con,$query7));
if(isset($_POST["submit"])){
  $agentid = $_POST["agentid"];
  $query4 = "UPDATE `agent` SET `AgentID`='$agentid' WHERE `EmpNo`='$empno'";
  $result4 = mysqli_query($con,$query4); 
  ?><h2 style="margin-left:50px;text-align:center">Successfully Edited</h2><?php
};
?>

<h1 id="signup">Edit Agent Information</h1>
<form class="row g-3" method="post">
  <div class="col-md-6">
    <label for="inputAgentID" class="form-label">Agent ID</label>
    <input type="number" name="agentid" class="form-control" id="inputAgentID" value="<?=$result7['AgentID']?>" required>
  </div>
