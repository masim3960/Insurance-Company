...
<body>
  ...
  $query7 = "SELECT * FROM `takes` WHERE `PolicyNo`='$polno' AND `NIC`='$NIC'";
  $result7 = mysqli_fetch_assoc(mysqli_query($con,$query7));
  $query8 = "SELECT * FROM `payment` WHERE `PolicyNo`='$polno' AND `PaymentNo`='$paymentno'";
  $result8 = mysqli_fetch_assoc(mysqli_query($con,$query8));
  $query9 = "SELECT * FROM `deals` WHERE`NIC`='$NIC'";
  $result9 = mysqli_fetch_assoc(mysqli_query($con,$query9));
if(isset($_POST["submit"])){
  $duration = $_POST["duration"];
  $amountpaid = $_POST["amountpaid"];
  $paymentdate = $_POST["paymentdate"];
  $dolastmeeting = $_POST["dolastmeeting"];
  $query4 = "UPDATE `deals` SET `D_O_LastMeeting`='$dolastmeeting' WHERE `NIC`='$NIC'";
  $result4 = mysqli_query($con,$query4);
  $query5 = "UPDATE `takes` SET `Duration`='$duration' WHERE `PolicyNo`='$polno' AND `NIC`='$NIC'";
  $result5 = mysqli_query($con,$query5);  
  $query6 = "UPDATE `payment` SET `AmountPaid`='$amountpaid',`payment_Date`='$paymentdate' WHERE `PolicyNo`='$polno' AND `PaymentNo`='$paymentno'";
  $result6 = mysqli_query($con,$query6);  
  ?><h2 style="margin-left:50px;text-align:center">Successfully Edited</h2><?php
};
?>

<h1 id="signup">Edit Customer Policy</h1>
<form class="row g-3" method="post">
  <div class="col-md-6">
    <label for="inputDuration" class="form-label">Duration</label>
    <input type="number" name="duration" class="form-control" id="inputDuration" value="<?=$result7['Duration']?>" required>
  </div>
  <div class="col-md-6">
    <label for="inputAmountPaid" class="form-label">Amount Paid</label>
    <input type="number" name="amountpaid" class="form-control" id="inputAmountPaid" value="<?=$result8['AmountPaid']?>" required>
  </div>
  <div class="col-md-6">
    <label for="inputPaymentDate" class="form-label">Payment Date</label>
    <input type="date" name="paymentdate" class="form-control" id="inputPaymentDate" value="<?=$result8['payment_Date']?>" required>
  </div>
  <div class="col-md-6">
    <label for="inputDOLastMeeting" class="form-label">Date Of Last Meeting</label>
    <input type="date" name="dolastmeeting" class="form-control" id="inputDOLastMeeting" value="<?=$result9['D_O_LastMeeting']?>" required>
  </div>
