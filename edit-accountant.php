...
<body>
  ...
  <?php
  $empno = $_GET['empno'];
  $query7 = "SELECT * FROM `accountant` WHERE `EmpNo`='$empno'";
  $result7 = mysqli_fetch_assoc(mysqli_query($con,$query7));
if(isset($_POST["submit"])){
  $deskno = $_POST["deskno"];
  $query4 = "UPDATE `accountant` SET `DeskNo`='$deskno' WHERE `EmpNo`='$empno'";
  $result4 = mysqli_query($con,$query4); 
  ?><h2 style="margin-left:50px;text-align:center">Successfully Edited</h2><?php
};
?>

<h1 id="signup">Edit Accountant Information</h1>
<form class="row g-3" method="post">
  <div class="col-md-6">
    <label for="inputDeskNo" class="form-label">Desk Number</label>
    <input type="number" name="deskno" class="form-control" id="inputDeskNo" value="<?=$result7['DeskNo']?>" required>
  </div>
