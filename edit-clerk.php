...
<body>
  ...
  <?php
  $empno = $_GET['empno'];
  $query7 = "SELECT * FROM `clerk` WHERE `EmpNo`='$empno'";
  $result7 = mysqli_fetch_assoc(mysqli_query($con,$query7));
if(isset($_POST["submit"])){
  $shifttiming = $_POST["shifttiming"];
  $query4 = "UPDATE `clerk` SET `ShiftTiming`='$shifttiming' WHERE `EmpNo`='$empno'";
  $result4 = mysqli_query($con,$query4); 
  ?><h2 style="margin-left:50px;text-align:center">Successfully Edited</h2><?php
};
?>

<h1 id="signup">Edit Clerk Information</h1>
<form class="row g-3" method="post">
  <div class="col-md-6">
    <label for="inputShiftTiming" class="form-label">Shift Timing</label>
    <input type="text" name="shifttiming" class="form-control" id="inputShiftTiming" value="<?=$result7['ShiftTiming']?>" required>
  </div>
